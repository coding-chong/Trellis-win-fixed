...
6. Normal Trellis roles must not use `acceptance.level: verified` or `gate`. Run any role-owned verification through the role's declared shell tool and record supported explicit evidence such as `commands-run` and `validation-output`; do not invoke the package host verification runner.
7. Do not use package-agent management or ambient global skills discovery as part of the Trellis role contract.
8. Do not test control actions, retry, rollback, parallelism, or package-only APIs in a production target. Stop on target drift, unknown migration customization, provider failure, task-identity drift, extension leakage, or telemetry secrecy failure.
...
| Safe control lifecycle | On a separate harmless async disposable child, obtain a `steer` receipt. On another disposable child, use `stop`, record terminal stopped lifecycle, and verify the expected resume refusal. A top-level async `workflow` `interrupt` request is intentionally unsupported by pi-subagents 0.46 and must be recorded as an expected rejection directing the caller to `stop`; it is not an enabled Trellis capability or an `environment-unavailable` result. | Steer request/delivery receipt; expected workflow-interrupt rejection text; stopped status/process-terminal evidence; resume rejection/error; no role recovery by resume; a later retry, if needed, is a new fresh run with a new explicit acceptance contract. |
...
The expected npm metadata for this release is `gitHead: 4a2d5284a2ac6a6b0282059e756fc5ee8dbdd58c` and `dist.integrity: sha512-hgldOVlaB05qXkQJRpp8wZCQ+TPZv4Xi+lu0Z2RYKRU3SaQmy3R9sCFM9myoqRSIgsLYKer41784BHbyh4QKIw==`; record the actual registry/tarball URL rather than assuming a registry mirror. Record the resolved source SHA, package version, source hash, exact commands, pass/fail/skip totals, named known platform baseline failures, and every new failure. A package-suite result is `PASS` only when the official suite completes and has no new or unexplained failures relative to that named baseline; known baseline-matched failures are `BASELINE-MATCHED`, not silently counted as clean. Do not modify the installed package, global Pi settings, or target project while running the harness.

For `pi-subagents@0.46.0` on this Windows host, the package harness is a known upstream risk audit rather than a Trellis integration veto. The observed suite is not clean (`1707 pass, 15 fail, 14 skipped`): three host-verification failures use an unquoted absolute executable path through `shell:true`; eleven agent discovery/management failures are coupled to Windows test project-root isolation; and one skills fallback failure leaks `APPDATA` into its test fixture. Record the package result as `KNOWN UPSTREAM RISK / NOT CLEAN`, retain all failures, and do not claim a package-suite PASS or silently baseline them. These package surfaces remain outside the enabled Trellis role contract.

Classify package surfaces such as custom worktrees, durable missions/schedules, public process-local RPC, structured delegation, prompt-workflow integration, capability ceilings, background-work providers, intercom/supervisor bridge behavior, package-agent management, and ambient global skills discovery as `package-harness-only` unless a future explicit Trellis contract enables them. The package harness may use only its own disposable environment; it must not turn these capabilities on in the target to make coverage look complete.

### Certification Decision

`Trellis integration certification: PASS` requires every `enabled-and-tested` and `intentionally-disabled-and-rejected` row to pass for the explicit role contract above. A package-harness result does not negate this integration result unless the failing package surface is enabled by that contract.

`Pi-subagents package harness on Windows: KNOWN UPSTREAM RISK / NOT CLEAN` records the separately pinned package audit. A new or unclassified package failure must be investigated and remains a package adoption risk, but does not require rolling back a passed Trellis integration target. Any target `FAIL` requires correction in the source fork or rollback in the disposable target.
